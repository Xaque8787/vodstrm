...
def _derive_path(
    ...
    if entry_type == "movie":
        ...
        return os.path.join(
            vod_root, settings.vod_movies_folder, label, f"{label}.strm"
        )
    ...
    if entry_type == "series":
        ...
        return os.path.join(
            vod_root, settings.vod_series_folder, t, season_dir, filename
        )
    ...
    if entry_type == "tv_vod":
        ...
        year_dir = (
            ...
            else settings.vod_unknown_year_folder
        )
        filename = f"{t} {air_date}.strm" if air_date else f"{t}.strm"
        return os.path.join(
            vod_root, settings.vod_series_folder, t, year_dir, filename
        )

    if entry_type == "live":
        return os.path.join(
            vod_root, settings.vod_live_tv_folder, f"{t}.strm"
        )

    # unsorted / fallback
    return os.path.join(
        vod_root, settings.vod_unsorted_folder, f"{t}.strm"
    )
...
def _cleanup_downloads(conn: sqlite3.Connection) -> int:
    ...
    expired = conn.execute(
        """
        SELECT entry_id, local_path, offline_path, staging_path
        FROM downloads
        WHERE (status='cancelled' AND cancelled_at IS NOT NULL
               AND cancelled_at < datetime(?, '-1 day'))
           OR (status='failed' AND failed_at IS NOT NULL
               AND failed_at < datetime(?, '-90 days'))
        """,
        (now, now),
    ).fetchall()
    for row in expired:
